...
Creates, updates, deletes, gets or lists a <code>cassandra_keyspaces</code> resource.
...
The Cassandra keyspace property was retrieved successfully.
...
<tr>
    ...
    <td><code>object</code></td>
    <td>The properties of an Azure Cosmos DB Cassandra keyspace</td>
</tr>
<tr>
...
The Cassandra keyspace properties were retrieved successfully.
...
```sql
INSERT INTO azure.cosmos_db.cassandra_keyspaces (
data__properties,
data__location,
data__tags,
data__identity,
...
- name: cassandra_keyspaces
  props:
    - name: subscriptionId
      value: string
      description: Required parameter for the cassandra_keyspaces resource.
...
```sql
DELETE FROM azure.cosmos_db.cassandra_keyspaces
WHERE subscriptionId = '{{ subscriptionId }}' --required
AND resourceGroupName = '{{ resourceGroupName }}' --required
AND accountName = '{{ accountName }}' --required
AND keyspaceName = '{{ keyspaceName }}' --required
